...
def _test_parameters(raversion, hspread, window_size, agent_volume, n_runs, n_steps, limit_p, market_p, cancel_p, informed_p, initial_midprice, initial_orders, volume_ran, offset_ran, step_std):
    ...
    for _ in range(n_runs):
        ...
        sim.initialise_agent(risk_aversion=raversion, half_spread=hspread, order_volume=agent_volume, var_window_size=window_size)
        sim.run()
...
class Optimiser:
    def __init__(self, limit_p, market_p, cancel_p, informed_p, initial_midprice, initial_orders, volume_ran, offset_ran, step_std, agent_volume):
        ...
        self.agent_volume = agent_volume
    ...
    def grid_search(self, n_values, n_runs, n_steps, raversion_ran, hspread_ran, window_size_ran):
        ...
        with ProcessPoolExecutor() as executor:
            for raversion, hspread, window_size in product(raversion_vals, hspread_vals, window_size_vals):
                args = (raversion, hspread, window_size, self.agent_volume, n_runs, n_steps, self.limit_p, self.market_p, self.cancel_p, self.informed_p, self.initial_midprice, self.initial_orders, self.volume_ran, self.offset_ran, self.step_std)
